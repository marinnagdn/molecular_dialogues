...
#get rid of [ and ] in bacteria's names
# for i in $(ls); do tr -d [ < $i > tmp && mv tmp $i ; done
# for i in $(ls); do tr -d ] < $i > tmp && mv tmp $i ; done
...
for i in $(ls *interactions.txt); do 
	...
	for line in $(cat $i | tail -n +2); do #tail -n +2 to ignore header first line
		...
		path1=$(find /home/marinna/M2/Bioinfo_appliquée_2/embl_gems-master/models -name "$genre1\_$species1*" | head -n 1 -q) #only first match, pas les autres souches #avec un _ entre le genre et l'espèce

		path2=$(find /home/marinna/M2/Bioinfo_appliquée_2/embl_gems-master/models -name "$genre2\_$species2*" | head -n 1 -q) #only first match, pas les autres souches
		...
		if [ ! -z $path1 ] && [ ! -z $path2 ]; then
			...
			python3 /home/marinna/M2/Bioinfo_appliquée_2/ACTION/list_interactions/get_bacterial_molecular_dialogues.py ${bacteria1_prefix_file} ${bacteria2_prefix_file} $(pwd)
